...
async def doctor_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"Врач: {doctor[1]}\nВыберите дату (календарь, только рабочие дни):", reply_markup=kb)
...
async def doctor_schedule(callback: CallbackQuery):
    ...
    if not user or user[2] != 'doctor':
        await callback.message.answer("Эта функция только для врачей.")
        return
    # Для демонстрации используем doctor_id = 1 (Мостовая)
    doctor_id = 1
    today = datetime.now().strftime("%Y-%m-%d")
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    ...
    conn.close()
    if not rows:
        await callback.message.answer("📋 Ваше расписание на сегодня:\n09:00 - Иванов И.И. (статус: подтверждено)\n11:30 - Петрова М.С. (статус: подтверждено)\n\n(демо-данные)")
    else:
...
@dp.message(Command("add_demo_data"))
async def add_demo_data(message: types.Message):
    chat_id = message.chat.id
    user = get_user_by_chat_id(chat_id)
    if not user or user[2] not in ('admin', 'deputy'):
        await message.answer("Только администратор или зам. директора могут добавить демо-данные.")
        return
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    # удалим старые демо-записи за последние 3 дня, чтобы не дублировать
    cur.execute("DELETE FROM appointments WHERE date IN (SELECT date('now'), date('now', '+1 day'), date('now', '+2 day'))")
    # найдём первого пациента
    cur.execute("SELECT id FROM users WHERE role='patient' LIMIT 1")
    patient_row = cur.fetchone()
    if not patient_row:
        await message.answer("Сначала зарегистрируйте хотя бы одного пациента (через /start).")
        conn.close()
        return
    ...
    if len(doctors) < 2:
        await message.answer("Недостаточно врачей.")
        conn.close()
        return
    doctor1 = doctors[0][0]
    doctor2 = doctors[1][0]
    ...
    demo = [
        (patient_id, doctor1, "Стоматология", today, "10:00", "confirmed"),
        (patient_id, doctor2, "УЗИ", today, "11:30", "confirmed"),
        (patient_id, doctor1, "Неврология", tomorrow, "09:00", "confirmed"),
        (patient_id, doctor2, "Терапия", tomorrow, "14:00", "missed"),
    ]
    for app in demo:
        cur.execute('''INSERT INTO appointments (user_id, doctor_id, service, date, time, status, created_at)
                       VALUES (?, ?, ?, ?, ?, ?, ?)''',
                    (app[0], app[1], app[2], app[3], app[4], app[5], datetime.now().isoformat()))
    conn.commit()
    conn.close()
    await message.answer("✅ Добавлены демо-записи на сегодня и завтра. Теперь отчёты и расписания будут непустыми.")
...
async def set_admin(message: types.Message):
    ...
    await message.answer("Ваша роль изменена на **Администратор**. Теперь в меню есть дополнительные кнопки.", reply_markup=main_menu('admin'))
...
async def set_doctor(message: types.Message):
    ...
    await message.answer("Ваша роль изменена на **Врач**. Теперь доступно расписание.", reply_markup=main_menu('doctor'))
...
async def set_deputy(message: types.Message):
    ...
    await message.answer("Ваша роль изменена на **Заместитель директора**. Теперь доступны отчёты.", reply_markup=main_menu('deputy'))
...
async def set_patient(message: types.Message):
    ...
    await message.answer("Ваша роль изменена на **Пациент**.", reply_markup=main_menu('patient'))
